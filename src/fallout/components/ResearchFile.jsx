...
export function ResearchFile({ feed, loading, now, onRetry }) {
  ...
  return <section className="fo-panel fo-research" id="axolotl" aria-labelledby="fo-axolotl-title">
    <div className="fo-file-tab" aria-hidden="true">R–03 / NATURAL SCIENCES</div>
    <PanelHeading number="03" label="BIOLOGICAL RESEARCH" status="FIELD FILE" />
    <div className="fo-research-content"><span className="fo-kicker">MONTHLY FISHING OBSERVATORY</span><h2 id="fo-axolotl-title" tabIndex={-1}><FalloutIcon name="axolotl" />A.X.O.L.O.T.L.</h2><div className="fo-specimen-label"><span>{specimen?.month || "SPECIMEN DATA PENDING"}</span><span>{current ? "ROTATION ACTIVE" : "UNCONFIRMED"}</span></div>
      <SpecimenImage key={`${specimen?.name}:${specimen?.imageUrl}`} specimen={specimen} />
      {specimen ? <><h3>{specimen.name}</h3><div className="fo-habitats"><FalloutIcon name="fishing" /><span>{specimen.regions.join(" + ")}</span></div><p className="fo-research-note">{current ? "This month’s rare catch. Find fishable water in the reported regions." : "Last known specimen. The current fishing rotation has not been confirmed."}</p>
      ...
      {reward ? <div className="fo-reward"><FalloutIcon name="reward" /><div><span className="fo-micro">RELATED MONTHLY REWARD</span><h4>{reward.title}</h4><p>{reward.description}</p><SourceLink source={reward.source} /><small>Bulletin reviewed {reward.reviewedAt}</small></div></div> : <p className="fo-research-note">Check Bethesda’s Atomic Shop bulletins for separately confirmed monthly rewards.</p>}
    </div>
