...
function publicScore(score, rank) {
  return { rank, discordId: String(score.discordId), username: score.username || "Cadet", avatarUrl: score.avatarUrl || DEFAULT_AVATAR_URL, bestScore: Number(score.bestScore) || 0, bestDurationMs: Number.isFinite(score.bestDurationMs) ? score.bestDurationMs : null, updatedAt: score.updatedAt || null };
}
...
export async function handleSpaceCadetPinballRequest(request, response) {
  const url = new URL(request.url || "/api/space-cadet-pinball", "http://localhost");
  const path = url.pathname.replace(/^\/api\/space-cadet-pinball\/?/, "");
