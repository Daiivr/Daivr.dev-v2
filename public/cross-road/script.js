...
function notifyCabinet(type, detail = {}) {
  if (window.parent && window.parent !== window) window.parent.postMessage({ type, ...detail }, window.location.origin);
}
...
const useGameStore = create((set, get) => ({
  ...
  endGame: () => {
    ...
    playerState.movesQueue = [];
    notifyCabinet("daivr:cross-score", { score: get().score, durationMs: Math.max(0, Date.now() - runStartedAt) });
    set({ status: "over" });
  },
  reset: () => {
